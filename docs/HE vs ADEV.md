...
┌─────────────────────────────────────────────────────────────────┐
│  bun (TypeScript 런타임)                                         │
│  역할: adev 자체 프로세스(하네스)를 실행하는 런타임              │
│  담당: adev 오케스트레이터 코드 실행                              │
│  대상: adev 자신 (autonomous-dev-agent-ts 코드베이스)            │
├─────────────────────────────────────────────────────────────────┤
│  Claude Agent SDK V2 (unstable_v2_createSession / prompt)       │
│  역할: 실제 개발 에이전트를 spawn하는 SDK                        │
│  담당: target project의 코드 작성, 테스트 생성/실행, 문서 작성  │
│  대상: 유저가 만들고자 하는 프로젝트 (target project)            │
└─────────────────────────────────────────────────────────────────┘
```
