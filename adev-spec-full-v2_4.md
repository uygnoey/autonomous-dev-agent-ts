# autonomous-dev-agent (adev) — 통합 스펙 v2.4
...
| 항목 | 값 |
|------|-----|
| **이름** | autonomous-dev-agent (CLI: `adev`) |
...
│  실행: V2 Session API + Agent Teams (env 활성화)    │
...
│   │   ├── agent-spawner.ts          ← V2 Session API + Agent Teams env spawn
...
│   │   ├── stream-monitor.ts         ← Hook 감시 + 디스크 IPC 폴링 (~/.claude/teams/ + tasks/) + 이상 패턴 탐지 [v2.2 변경, v2.4 디스크IPC 추가]
...
**기술 구현**: adev가 이벤트 발생 시 documenter를 unstable_v2_prompt()로 spawn → 문서 생성 → 종료
```
...
**방법 ①: V2 Session API + Agent Teams — 전체 에이전트 실행** [v2.4 변경]

```typescript
import { unstable_v2_createSession } from "@anthropic-ai/claude-code";

// V2 Session API + Agent Teams 활성화
const session = unstable_v2_createSession({
  systemPrompt: "아키텍처 설계: [스펙 내용]",
...
|-------|----------|-------------|------|
| DESIGN | session.stream() 1개 | ✅ 활성화 | 팀 토론 필요 (architect/qa/coder/reviewer 실시간 의논) |
| CODE | unstable_v2_prompt() N개 동시 (Promise.all) | ❌ 불필요 | coder 간 독립 병렬 (대화 불필요) |
| TEST | unstable_v2_prompt() 순차 | ❌ 불필요 | Fail-Fast 순차 실행 |
| VERIFY | unstable_v2_prompt() 순차 | ❌ 불필요 | 4중 검증 순차 실행 |
...
adev (TypeScript 프로세스) = Team Leader
  │
  ├─ DESIGN: session.stream() 1개 + Agent Teams env
  │   → lead agent가 teammate spawn (TeamCreate → Task)
  │   → teammate 간 SendMessage 실시간 토론
  │   → adev는 Hook + 디스크 IPC 폴링 모니터링 + 이상 시 개입
  │
  ├─ CODE: unstable_v2_prompt() N개 동시 (Promise.all)
  │   → 각 coder 독립 모듈 구현
  │   → architect/reviewer 감독 별도 세션
  │   → adev가 결과 수집/검증
  │
...
### 8.3 adev 직접 제어 + Hook/디스크IPC 감시 [v2.2 변경, v2.4 모니터링 방식 확정]

adev(TypeScript 프로세스)가 **직접 오케스트레이터**:

```
[DESIGN 페이즈 — Agent Teams 활용]
session.stream() 1개 호출 → lead agent가 팀 구성 + 토론 주도
adev는 Hook + 디스크 IPC 폴링 모니터링:
  → 정상: 개입 안 함
  → 이상 감지: 세션 종료 → 새 세션으로 강제 지시

[CODE/TEST/VERIFY 페이즈 — 독립 세션]
adev가 직접 unstable_v2_prompt() 호출/관리:
...
**adev 개입 방법:**
- 세션 종료 후 새 세션으로 강제 지시
- 수정된 프롬프트로 재spawn
- 극단적: Agent Teams 비활성화하고 독립 unstable_v2_prompt()로 전환
...
│                                                       │
│  실행: session.stream() 1개 + Agent Teams env 활성화 │
...
│  adev: Hook + 디스크 IPC 폴링 모니터링, 이상 패턴 감지  │
│                                                       │
...
┌─── Phase 2: CODE (병렬 코딩) ────────────────────────┐
│                                                       │
│  실행: unstable_v2_prompt() N개 동시 (Promise.all)   │
│  참여: coder×N + architect(감독) + reviewer(감독)      │
│  + documenter: 이벤트 트리거 — 코드 완료 시 spawn하여 문서 생성 [v2.3]  │
│                                                       │
│  coder×N: 각각 독립 세션으로 모듈별 병렬 구현            │
│  architect: 별도 세션으로 설계 준수 감독                  │
│  reviewer: 별도 세션으로 코드 품질 감독                   │
...
```
1순위: V2 Session API 세션 내 컨텍스트
  → DESIGN: Agent Teams SendMessage (팀 토론)
  → CODE/TEST/VERIFY: unstable_v2_prompt() 독립 세션 + 파일 시스템 공유
  → 실시간 의논, 짧은 피드백, 상태 전달
  → 제약: 컨텍스트 윈도우 크기 한계
...
[2계층-A] 기능 단위 개발 — V2 Session API + Agent Teams
  모듈 분배: DESIGN 페이즈에서 Agent Teams 토론 → adev spawn 순서
  기능별 루프:
    Phase 1 DESIGN: session.stream() + Agent Teams (팀 토론), adev Hook+디스크IPC 감시
    Phase 2 CODE: unstable_v2_prompt()×N 동시 (Promise.all), Git branch 격리, adev 감시
...
- [x] Agent Teams 실험적 기능 안정화 대응 → V2 Session API primary, Agent Teams는 DESIGN만, PoC 성공으로 대안 불필요 [v2.4 갱신]
...
**목적**: V2 Session API 내에서 Agent Teams가 실제로 잘 동작하는지, 성능 한계는 어디인지 측정

**검증 항목**:
1. Agent Teams spawn 오버헤드 (TeamCreate → Task 시간)
2. teammate 동시 수 한계 (3명, 5명 검증 완료; 7명 미검증)
3. 동시 unstable_v2_prompt() 안정성 (Promise.all로 3~5개)
...
**성공 기준**:
- spawn 오버헤드: 5초 이내
- teammate 동시 5명: 안정 동작 (7명은 미검증, 추후 확인 필요)
- concurrent unstable_v2_prompt() 5개: 모두 정상 완료
- SendMessage 지연: 2초 이내

**실패 시 대안**: Agent Teams 비활성화 → DESIGN Phase도 독립 unstable_v2_prompt()로 전환

**[v2.4] 실제 결과**: ✅ 전부 성공
- spawn 오버헤드: 콜드스타트 ~24s, 이후 ~7s (정상 범위)
- teammate 동시 5개: 안정 동작 확인
- concurrent unstable_v2_prompt() 5개: 전부 성공, 드랍아웃 없음
...
**목적**: Agent Teams 내부 SendMessage 내용을 adev가 모니터링할 수 있는지 확인 (스트림 + 디스크 IPC)

**검증 항목**:
1. teammate 간 SendMessage가 Hook/스트림에서 캡처되는지
2. 디스크 IPC (inbox JSON)로 메시지 내용 파싱 가능한지
3. Hook + 디스크 IPC 폴링 조합으로 완전 가시성 확보 가능한지
...
bun install
bun run test-message-visibility.ts  # Hook + 스트림 메시지 캡처 확인
bun run test-stream-parsing.ts      # 디스크 IPC inbox JSON 파싱 가능성
```

**성공 기준**:
- Hook 또는 디스크 IPC 중 하나 이상에서 메시지 내용 확인 가능
- 메시지 발신자/수신자/내용 파싱 가능

**실패 시 대안**: 
- Agent Teams 결과를 LanceDB에 기록 → adev가 폴링
- 또는 Agent Teams 비활성화 후 독립 unstable_v2_prompt()로 전환
